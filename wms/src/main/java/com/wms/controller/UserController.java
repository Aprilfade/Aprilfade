...
public class UserController {
    ...
    public Result listPageC1(@RequestBody QueryPageParam query) {
        ...
        // 2. 解析查询条件（使用 LambdaQueryWrapper 更安全）
        LambdaQueryWrapper<User> lambdaQueryWrapper = new LambdaQueryWrapper<>();
        ...
        if (name != null && !name.isEmpty() && !"null".equals(name)) {
            lambdaQueryWrapper.like(User::getName, name); // 按实体类字段名查询
        }
        if (sex != null && !sex.isEmpty() && !"null".equals(sex)) {
            lambdaQueryWrapper.eq(User::getSex, sex); // 按实体类字段名查询
        }
        if (roleId != null && !roleId.isEmpty() && !"null".equals(roleId)) {
            lambdaQueryWrapper.eq(User::getRoleId, roleId); // 按实体类字段名查询
        }

        // 3. 调用标准分页方法（MyBatis-Plus 原生方法）
